...
- Il sito legge da **`public`**, dove ci sono **38 finestre sottili** `security_invoker`, non dallo schema `caprera` (ADR-002: una spunta nel cruscotto non si versiona). **Contratti, finanze e mister non hanno finestra**; l'unica eccezione è `public.contratti_pubblici`, senza clausola né ingaggio. Aprirne una nuova si fa in `02-viste.sql` e passa dal Direttore Sportivo — **mai dal cruscotto**.
