...
- **Supabase è collegato e l'archivio ci vive dentro** (dal 21/08/2026): progetto `caprera` (`ziggietzdtdtpsfmpthm`), Postgres 17, schema `caprera` con **23 tabelle, 10 viste, 6 funzioni**, regole di riga ovunque.
- ⚠️ **Il tuo primo lavoro, prima di ogni altra cosa:** `01/02/03.sql` sono fermi al 20/08 e **non** contengono né le 33 finestre di `public` né la macchina delle tessere (tabella, trigger, `attiva_tessera`, `attiva_la_mia_tessera`, `la_mia_tessera`). Quella parte vive **solo nel progetto Supabase**: il database **non si ricrea dai file**. Va riportata nei file versionati. È il rischio di ADR-002 rientrato dalla finestra il giorno dopo.
...
- Il sito legge da **`public`**, dove ci sono **33 finestre sottili** `security_invoker`, non dallo schema `caprera` (ADR-002: una spunta nel cruscotto non si versiona). **Contratti, finanze e mister non hanno finestra**; l'unica eccezione è `public.contratti_pubblici`, senza clausola né ingaggio. Aprirne una nuova si fa in `02-viste.sql` e passa dal Direttore Sportivo — **mai dal cruscotto**.
...
- Lo schema **`vecchio_progetto`** (tentativo di set. 2025 tolto da `public`) **non si cancella**: dentro ci sono i 48 alias di nomi scritti a mano che possono agganciare parte delle 111 righe di rosa rimaste orfane su 2.999. Prima li recuperi, poi eventualmente si droppa.
