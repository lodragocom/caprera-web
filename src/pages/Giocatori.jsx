...
import { rosters, teamName } from '../lib/data'
...
const INDICE = (() => {
  ...
  for (const r of rosters) {
    const cur = acc.get(r.player) ?? {
      player: r.player, roles: new Set(), teams: new Set(),
    ...
    cur.spent += r.cost ?? 0
    cur.apps += r.apps ?? 0
    if (r.fm != null) { cur.fmSum += r.fm; cur.fmN += 1 }
    if (cur.best === null || (r.cost ?? 0) > cur.best) cur.best = r.cost ?? 0
